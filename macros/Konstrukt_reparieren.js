...
(async () => {
  ...
  const destroyedAt = Number(companion.getFlag("alkenstern-vtt", "destroyedAt") ?? 0);
  if (destroyedAt > 0) {
  ...
  const now = Date.now();
  let breakEvents = companion.getFlag("alkenstern-vtt", "breakEvents") ?? [];
  breakEvents = Array.isArray(breakEvents)
    ? breakEvents.map((n) => Number(n)).filter((n) => Number.isFinite(n) && (now - n) <= BREAK_WINDOW_MS)
    : [];

  if (hp.value <= 0) {
    breakEvents.push(now);

    if (breakEvents.length > 2) {
      await companion.setFlag("alkenstern-vtt", "breakEvents", breakEvents);
      await companion.setFlag("alkenstern-vtt", "destroyedAt", now);
    ...
    buttons: {
      repair: {
        ...
        callback: async (html) => {
          ...
          if (after > 0) {
            await companion.unsetFlag("alkenstern-vtt", "destroyedAt");
          }
